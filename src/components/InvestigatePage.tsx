...
function ReportView({
  d, graph, expandingKey, onExpand, onCollapse,
}: {
  d: Dossier
    ...
    <IsoPageShell>
      ...
      {q && !loading && !error && dossier && (
        dossier.note ? <NonRoutable d={dossier} /> : (
          <ReportView d={dossier} graph={graphState} expandingKey={expandingNode} onExpand={expandNode} onCollapse={collapseToDepth} />
        )
      )}
      {crumbs.length > 1 && (
