 ...
 * The answer is ~200 bytes. This function runs the same scanIndicatorLogic the
 * MCP/REST scan paths use (one source of truth for verdicts), caches each
 * verdict in IOC_CACHE (1 h TTL — the daily pipeline rewrite plus the disputes
 * table are the freshness ceiling we accept), and serves the hit from KV.
...
export const onRequestGet = async (context: any) => {
  ...
  // 'latest' matches the MCP server's feed-version convention: the per-isolate
  // scanner cache lives as long as the isolate (minutes of idle), which is
  // shorter than the daily pipeline cycle anyway.
  let result: unknown
  try {
    result = await scanIndicatorLogic(value, 'latest')
  } catch (err: any) {
