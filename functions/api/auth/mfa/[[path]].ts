...
const onRequestAny = async (context: any) => {
  ...
  // Removing a factor is not a read, but we do NOT pre-empt it with an aal check
  // here. GoTrue's rule is narrower than "aal1 can't unenroll": only the last
  // VERIFIED factor requires aal2, while unverified / abandoned setups are always
  // removable. A blanket edge gate would strand a user trying to clean up a
  // half-finished enrollment. Instead we forward the DELETE and let mapMfaError
  // translate GoTrue's 422 `insufficient_aal` into an honest 403 `aal_required`
  // ("Verify your two-factor code...") rather than the opaque body it returns.
    ...
    return json(
      {
        ...
        retry: mapped.retry || undefined,
        aal_required: mapped.aal_required || undefined,
      },
      mapped.status,
      request,
    )
